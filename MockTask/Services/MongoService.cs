...
namespace MockTask.Services
{
    public class MongoService
    {
        ...
        public MongoService(IOptions<DatabaseSettings> _databaseSettingsModel)
        {
            ...
            _classMongo = mongoDatabase.GetCollection<Classes>(_databaseSettingsModel.Value.CollectionNames[0]);
            _studentMongo = mongoDatabase.GetCollection<Students>(_databaseSettingsModel.Value.CollectionNames[1]);
            _subjectMongo = mongoDatabase.GetCollection<Subjects>(_databaseSettingsModel.Value.CollectionNames[2]);
            _teacherMongo = mongoDatabase.GetCollection<Teachers>(_databaseSettingsModel.Value.CollectionNames[3]);
